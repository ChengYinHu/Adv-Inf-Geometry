...
import os
from functions import initiation, line_pso, clip, ellipse_pso, triangle_pso
...
for K in range(1, 2):
    ...
    for pic in file:
        ...
        population = np.zeros((seed, (K + 1) * num_g))
        unit = np.zeros((1, (K + 1) * num_g))
        ...
        P_best = np.zeros((seed, (K + 1) * num_g))
        conf_p = np.ones((1, seed)) * 100
        G_best = np.zeros((1, (K + 1) * num_g))
        conf_G = 100
        V = np.zeros((seed, (K + 1) * num_g))
        ...
        for steps in range(step):
            ...
            for seeds in range(seed):
                ...
                line_pso(img_inf_path, population[seeds], path_adv_inf, K+1, R, num_g)
                # ellipse_pso(img_inf_path, population[seeds], path_adv_inf, R)
                # triangle_pso(img_inf_path, population[seeds], path_adv_inf, K, R)
...
print(ASR[1] / count_all[1])
print(Query[1] / count_all[1])
